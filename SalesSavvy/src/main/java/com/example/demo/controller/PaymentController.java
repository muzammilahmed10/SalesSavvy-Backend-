...
public class PaymentController {
	...
	public ResponseEntity<String> createPaymentOrder(@RequestBody Map<String, Object> requestBody, HttpServletRequest request) {
		try {
			...
			// Extract totalAmount and cartItems for the request body
			...
			List<OrderItem> cartItems = cartItemsRaw.stream().map(item -> {
				...
				BigDecimal pricePerUint = new BigDecimal(item.get("price").toString());
				orderItem.setPricePerUnit(pricePerUint);
				orderItem.setTotalPrice(pricePerUint.multiply(BigDecimal.valueOf((Integer) item.get("quantity"))));
				return orderItem;
	...
	public ResponseEntity<String> verifyPayment(@RequestBody Map<String, Object> requestBody, HttpServletRequest request) {
		try {
			...
			// Call the payyment service to verify the payment
			...
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error verfying payment: "+e.getMessage());
		}
	}
